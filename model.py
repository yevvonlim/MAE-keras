...
# ----------------- Encoder -------------------
    for _ in range(N_e):
        x = TransformerBlock(num_heads=num_heads, mlp_dim=dff, dropout=drop, dtype=tf.float16) (x) 
    mst = MaskToken()
    x = mst (x, random_sampler.mask_indices, random_sampler.un_masked_indices)

# -------------- Positional Embedding _-------------------
    x = Lambda(lambda x: x + pos_encode(x.shape[1], d_model)) (x)  
    x = Dense(units=d_decoder, dtype=tf.float16) (x)
# ----------------- Decoder -------------------
    for _ in range(N_d):
        x = TransformerBlock(num_heads=num_heads, mlp_dim=dff_decoder, dropout=drop, dtype=tf.float16) (x)
    x = Dense(units=patch_size*patch_size*3, dtype=tf.float16) (x)
